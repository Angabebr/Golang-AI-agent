...
1. navigate - перейти на URL
   - Заполни: "url" (из списка links.href)
...
3. fill - заполнить поле ввода
   - ОБЯЗАТЕЛЬНО заполни: "text" (placeholder или name из списка inputs)
...
- Для действия "fill": ВСЕГДА заполняй "text" (из inputs.placeholder или inputs.name) И "value"
- Для действия "navigate": ВСЕГДА заполняй "url" из списка links.href
- НЕ используй заготовленные селекторы - анализируй ТОЛЬКО данные текущей страницы
- НЕ используй общие знания о сайтах - работай только с тем, что видишь
...
Для навигации:
{
  "action": "navigate",
  "reasoning": "объяснение",
  "url": "ОБЯЗАТЕЛЬНО: полный URL из списка links"
}
